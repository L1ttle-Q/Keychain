import os, ast, subprocess, pyperclip
...
def pyperCopy(URL):
    ...
    for Acc, Pas in KeySet[URL].items():
        print(f"{cnt}: [Account:{Acc}\nPassWord:{Pas}]\n")
        tmp.setdefault(cnt)
        tmp[cnt] = {Acc: Pas}
        cnt += 1
    cnt = int(input("Input the number before the account you want to use."))
    if (auto.confirm("Do you want to copy it?") == "OK"):
        for Acc, Pas in tmp[cnt].items():
            if (auto.confirm("You can click \"OK\" to copy the account.") == "OK"):
                pyperclip.copy(str(Acc))
            if (auto.confirm("You can click \"OK\" to copy the password.") == "OK"):
                pyperclip.copy(str(Pas))
    ...
    if (opt == 'q'):
        ...
        if (KeySet):
            ...
            for U, A in KeySet.items():
                print(f"{cnt}: {U}")
                cnt += 1
            tmp = int(input("Input the number before the site you want to find."))
            cnt = 1
            for U, A in KeySet.items():
                if (cnt == tmp):
                    pyperCopy(U)
